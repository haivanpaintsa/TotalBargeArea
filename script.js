...
const selfPropelledElements = Array.from(document.querySelectorAll(".self-propelled-section, .self-propelled-only"));
...
function validateCargoHold(showErrors = true) {
    ...
    const partitionCheck = validateNonNegativeIntegerField("partition-count", "Số vách ngăn", showErrors);
...
function calculateSelfPropelled(input, walkwayWidth) {
    ...
    const bottomAndBilge = input.length * input.width * draft * input.coefficientK;
    const drySide = (input.length + input.width) * freeboard * 2;
    const walkway = input.length * walkwayWidth * 2;
    const coaming = input.hasCoaming
        ? ((input.coamingLength * input.coamingHeight) + (input.coamingWidth * input.coamingHeight)) * input.coamingFactor * 2
        : 0;
    const cargoHold = input.hasCargoHold
        ...
        : 0;
    const inside = (bottomAndBilge + drySide + (input.length * input.width)) * input.ribFactor;
    const total = bottomAndBilge + drySide + walkway + coaming + cargoHold + inside;

    return { draft, freeboard, bottomAndBilge, drySide, walkway, coaming, cargoHold, inside, total };
}
...
function renderResults(result = null) {
    ...
    config.resultRows.forEach(([key, label]) => {
        resultsBody.appendChild(createResultRow(label, result ? result[key] : null));
    });
    resultsBody.appendChild(createResultRow("Tổng diện tích", result ? result.total : null, true));

    const hasResult = Boolean(result) && Object.values(result).every(Number.isFinite);
...
function renderFormulaDetails(input = null, result = null) {
    ...
    variableText.textContent = "Biến sử dụng: D = chiều dài; R = chiều rộng; C = chiều cao; T₀ = mớn quy ước; H = chiều cao mạn khô; k = hệ số hiệu chỉnh.";
    formulaContent.appendChild(variableText);

    if (!input || !result) {
        const emptyText = document.createElement("p");
        emptyText.textContent = "Nhập đủ dữ liệu để hiển thị phép thế số cho từng khu vực.";
    ...
    if (selectedBargeType === "deck") {
        ...
        commonBlocks.push(
            ...
            createFormulaBlock("Quầy hầm", input.hasCoaming ? [
            ...
            createFormulaBlock("Hầm hàng", input.hasCargoHold ? [
    ...
    commonBlocks.push(createFormulaBlock("Tổng diện tích", [
...
function getInputSummary(input) {
    const entries = [
        ...
        ["Mớn quy ước T₀", selectedBargeType === "deck" ? input.height / 5 : input.height / 4, "m"],
        ["Chiều cao mạn khô H", selectedBargeType === "deck" ? (4 * input.height) / 5 : (3 * input.height) / 4, "m"],
    ];
...
function updatePrintMeta(input, calculatedAt) {
    document.getElementById("print-barge-type").textContent = BARGE_TYPES[selectedBargeType].name;
    document.getElementById("print-calculated-at").textContent = DATE_TIME_FORMATTER.format(calculatedAt);
    const list = document.getElementById("print-input-list");
    list.replaceChildren();

    getInputSummary(input).forEach(([label, value, unit]) => {
        const item = document.createElement("li");
        const formattedValue = typeof value === "number" ? formatArea(value) : value;
        item.textContent = `${label}: ${formattedValue}${unit ? ` ${unit}` : ""}`;
        list.appendChild(item);
    });
}

function performCalculation({ showErrors = true, focusFirstError = false, scrollToResults = false } = {}) {
    ...
    if (!Object.values(result).every(Number.isFinite)) {
        lastCalculation = null;
        renderResults();
        resultStatus.textContent = "Không thể tính với dữ liệu hiện tại. Vui lòng kiểm tra lại.";
        return false;
    }
...
function updateAutomaticFields(draftValue, freeboardValue) {
    getInput("draft").value = Number.isFinite(draftValue) ? formatArea(draftValue) : "";
    getInput("freeboard").value = Number.isFinite(freeboardValue) ? formatArea(freeboardValue) : "";
}
...
function updateAutomaticFieldsFromHeight() {
    ...
    const isValid = isStrictNumericText(heightInput.value) && Number.isFinite(height) && height > 0;
...
function updateBargeTypeUI() {
    ...
    const activeTab = typeTabs.find((tab) => tab.dataset.bargeType === selectedBargeType);
    ...
    const ruleText = selectedBargeType === "deck"
        ? "T₀ = C / 5 · H = 4 × C / 5"
...
function resetForm() {
    ...
    getInput("rib-factor").value = String(BARGE_TYPES[selectedBargeType].ribFactor);
    clearAllErrors();
    updateBargeTypeUI();
    updateAutomaticFields(null, null);
    lastCalculation = null;
...
function buildCopyText() {
    ...
    lines.push("", "Lưu ý: Kết quả mang tính chất ước tính; cần kiểm tra thông số thực tế trước khi sử dụng.");
    return lines.join("\n");
}
...
form.addEventListener("submit", (event) => {
    ...
    performCalculation({ showErrors: true, focusFirstError: true, scrollToResults: true });
});
...
typeTabs.forEach((tab, index) => {
    tab.addEventListener("click", () => handleBargeTypeChange(tab.dataset.bargeType));
    tab.addEventListener("keydown", (event) => {
        ...
        if (event.key === "ArrowLeft") nextIndex = (index - 1 + typeTabs.length) % typeTabs.length;
...
document.getElementById("load-sample-button").addEventListener("click", loadSampleData);
